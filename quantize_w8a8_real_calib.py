# -*- coding: utf-8 -*-
"""
W8A8 INT8 Quantization with Real Calibration Data

使用PyTorch原始模型的输出作为校准数据，确保量化精度。
生成两种模型：
  - INT8模型 → onnx_models/int8/ (1D scale, ORT CPU兼容)
  - NPU模型 → onnx_models/int8/optimized_npu/ (多维scale, WebNN兼容)
...
MODELS = [
    ...
    },
    {
        ...
        'quantizable': True,
        'calib_samples': 50,  # More samples for vocoder precision
...
def generate_real_calibration_data(wrapper, inputs_spec, num_samples=20, device='cpu'):
    """Generate calibration data using real PyTorch model outputs.
    
    This ensures the calibration data matches the actual distribution
    the model will see during inference.
    """
    print(f"    Generating {num_samples} real calibration samples...")
    feeds_list = []
    ...
    for i in range(num_samples):
        ...
        for name, (dtype, shape) in inputs_spec.items():
            if dtype == torch.long:
                feeds[name] = torch.randint(0, 255, shape, dtype=torch.long, device=device)
            elif name == 't':
                feeds[name] = torch.rand(*shape, dtype=dtype, device=device)
            elif name == 'xt_mask':
                feeds[name] = torch.ones(*shape, dtype=dtype, device=device)
            else:
                feeds[name] = torch.randn(*shape, dtype=dtype, device=device)
        
        # Run through PyTorch model to get realistic distributions
        with torch.no_grad():
            try:
                _ = wrapper(*[feeds[k] for k in inputs_spec.keys()])
            except Exception as e:
                print(f"      Warning: PyTorch inference failed for sample {i}: {e}")
...
def quantize_w8a8_ort(fp32_onnx_path, int8_onnx_path, model_name, calibration_data, 
                      per_channel=False, num_samples=20):
    ...
    print(f"    Quantizing W8A8 with ORT (QDQ format, per_channel={per_channel})...")
...
def process_one_model(model_cfg, pytorch_model, config):
    """Process a single model: export FP32 → calibrate → quantize W8A8 → optimize."""
    name = model_cfg['name']
    wrapper_name = model_cfg['wrapper']
    inputs_spec = model_cfg['inputs']
    is_quantizable = model_cfg['quantizable']
    calib_samples = model_cfg.get('calib_samples', 20)
    ...
    # 3. Generate real calibration data
    if is_quantizable:
        calib_data = generate_real_calibration_data(wrapper, inputs_spec, 
                                                     num_samples=calib_samples, device=DEVICE)
    else:
        calib_data = None
    ...
    # 4. Quantize W8A8 with real calibration data
    int8_raw_path = os.path.join(INT8_DIR, f'{name}_raw.onnx')
    if is_quantizable:
        quantize_w8a8_ort(fp32_path, int8_raw_path, name, calib_data,
                         per_channel=per_channel, num_samples=calib_samples)
    else:
        print(f"  Non-quantizable (Embedding/STFT) — re-saving FP32")
    ...
    # 5. Optimize for NPU (WebNN: fix DQL scale rank)
    ...
    # 6. Optimize for INT8 (ORT CPU: keep 1D scales)
...
def main():
    parser = argparse.ArgumentParser(description='W8A8 PTQ Export Pipeline with Real Calibration')
    parser.add_argument('--model-path', default=None,
                        help='Path to SoulX-Singer model.pt')
    args = parser.parse_args()

    print("=" * 60)
    print("W8A8 PTQ Export Pipeline (Real Calibration)")
