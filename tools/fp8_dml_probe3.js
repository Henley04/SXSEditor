// Diagnostic v3: correctly detect FP16 via array-style metadata, then test DML.
...
async function probe(key) {
    ...
    // Probe input type via CPU session (array-style access!)
    let cpuSess;
    try {
    ...
    console.log(`  Input type: ${inputType} (name: ${cpuSess.inputMetadata[0]?.name})`);
    cpuSess.release();
    ...
    // DML session + run (exactly like createSessionWithValidation does)
    let dmlSess = null;
    try {
    ...
    try {
        ...
        console.log(`  >>> DML CANNOT run this FP8 model — falls back to CPU`);
    }
    dmlSess.release();
}
